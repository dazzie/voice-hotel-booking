...
def _voice_agent_reply(
    ...
    with lock:
        ...
        elif was_barge_in and _is_probable_playback_echo(transcript):
            suppress = True
            reason = "probable_playback_echo"
        if suppress:
            trace.event(f"audio.{reason}", transcript=transcript)
            return _finish_response(
                agent,
                trace,
                "",
...
def _is_probable_playback_echo(transcript: str) -> bool:
    ...
    if not normalized:
        return True
    if normalized in _ECHO_PHRASES:
        return True
