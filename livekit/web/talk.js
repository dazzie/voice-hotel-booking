...
function speakWithBrowserVoice(text, locale, token) {
  ...
  utterance.onend = () => { clearInterval(ttsKeepAlive); finishAgentPlayback(token); };
  utterance.onerror = () => { clearInterval(ttsKeepAlive); finishAgentPlayback(token); };
...
function thresholds() {
  ...
  return {
    ...
    barge: Math.min(0.14, Math.max(0.03, start * 1.7, playbackEchoFloor * 2.0)),
  };
}
...
function startTurnRecording(isBargeIn = false) {
  ...
  recorder.onstop = () => {
    ...
    const turnDuration = Date.now() - recordingStartedAt;
    if (shouldDiscard || audioBlob.size < 800 || turnDuration < 400) {
...
async function sendAudioToAgent(audioBlob) {
  ...
  try {
    ...
    if (payload.ignored) {
      ...
      renderSources([]);
      consecutiveBargeIns = 0;
      listenCooldownUntil = Date.now() + 2000;
      agentBusy = false;
    ...
    if (wasBargeIn) {
      consecutiveBargeIns += 1;
      if (consecutiveBargeIns >= 2) {
        voicePlaceholder.remove();
        appendRuntimeEvent(`barge_in.loop_suppressed | ${consecutiveBargeIns} consecutive`);
        listenCooldownUntil = Date.now() + 3000;
        agentBusy = false;
        setListeningState("Listening", "Echo loop suppressed. Continue speaking naturally.");
        return;
      }
    } else {
      consecutiveBargeIns = 0;
